...
const materials = [
  { id: 1, name: "Premium Cement", brand: "Dangote", price: 4500, rating: 4.9, reviews: 2341, image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80", verified: true, type: "material" },
  { id: 2, name: "Steel Rods 12mm", brand: "BUA Steel", price: 8500, rating: 4.8, reviews: 1892, image: "https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?w=400&q=80", verified: true, type: "material" },
  { id: 3, name: "Roofing Sheets", brand: "Aluminum Co", price: 12000, rating: 4.7, reviews: 987, image: "https://images.unsplash.com/photo-1632759145351-1d592919f522?w=400&q=80", verified: true, type: "material" },
];

const generalContractors = [
  { id: 101, name: "BuildRight Contractors", specialty: "Residential & Commercial", rating: 4.9, reviews: 156, projects: 89, image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&q=80", verified: true, hiringFee: 5000, type: "gc" },
  { id: 102, name: "Elite Construction", specialty: "Luxury Homes", rating: 4.8, reviews: 124, projects: 67, image: "https://images.unsplash.com/photo-1506277886164-e25aa3f4ef7f?w=400&q=80", verified: true, hiringFee: 7500, type: "gc" },
];

const subContractors = [
  { id: 201, name: "PowerLine Electricals", specialty: "Electrical Installation", rating: 4.9, reviews: 234, image: "https://images.unsplash.com/photo-1531384441138-2736e62e0919?w=400&q=80", verified: true, hiringFee: 2500, type: "sub" },
  { id: 202, name: "AquaFlow Plumbing", specialty: "Plumbing & Sanitation", rating: 4.8, reviews: 189, image: "https://images.unsplash.com/photo-1522529599102-193c0d76b5b6?w=400&q=80", verified: true, hiringFee: 2000, type: "sub" },
  { id: 203, name: "CoolAir HVAC", specialty: "Air Conditioning", rating: 4.7, reviews: 156, image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&q=80", verified: true, hiringFee: 3000, type: "sub" },
];

type CartItem = { id: number; name: string; price?: number; hiringFee?: number; quantity: number; type: string; image: string; };
...
export default function ShopScreen() {
  ...
  const filteredGCs = generalContractors.filter(gc => 
    gc.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    gc.specialty.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const addToCart = (item: any) => {
    ...
    if (existingItem) {
      ...
      setCart([...cart, { id: item.id, name: item.name, price: item.price, hiringFee: item.hiringFee, quantity: 1, type: item.type, image: item.image }]);
    }
  };

  const removeFromCart = (id: number) => setCart(cart.filter(c => c.id !== id));
  const updateQuantity = (id: number, delta: number) => setCart(cart.map(c => c.id === id && c.type === 'material' ? { ...c, quantity: Math.max(1, c.quantity + delta) } : c));
  ...
  return (
    <View className="flex-1 bg-white">
      <View className="pt-16 px-6 pb-4 flex-row items-center justify-between">
        ...
        <Text className="text-2xl text-black" style={{ fontFamily: 'Poppins_700Bold' }}>Shop</Text>
      ...
      <ScrollView className="flex-1 px-6" contentContainerStyle={{ paddingBottom: 100 }}>
        {activeSection === 'materials' && <View className="pb-8">
          <Text className="text-xl text-black mb-4" style={{ fontFamily: 'Poppins_700Bold' }}>Construction Materials</Text>
          {filteredMaterials.map((item) => (
            <View key={item.id} className="bg-white rounded-3xl mb-4 overflow-hidden border border-gray-200 flex-row">
              <Image source={{ uri: item.image }} className="w-28 h-28 bg-gray-100" resizeMode="cover" />
              <View className="flex-1 p-4 justify-center">
                <View className="flex-row items-center mb-1"><Text className="text-base text-black" style={{ fontFamily: 'Poppins_700Bold' }}>{item.name}</Text>{item.verified && <View className="ml-2 bg-black rounded-full px-2 py-0.5"><Text className="text-white text-xs" style={{ fontFamily: 'Poppins_600SemiBold' }}>✓</Text></View>}</View>
                ...
                <View className="flex-row items-center justify-between">
                  ...
                  <TouchableOpacity onPress={() => isInCart(item.id) ? removeFromCart(item.id) : addToCart(item)} className={`px-3 py-2 rounded-xl ${isInCart(item.id) ? 'bg-gray-200' : 'bg-black'}`}><Text className={`text-xs ${isInCart(item.id) ? 'text-black' : 'text-white'}`} style={{ fontFamily: 'Poppins_600SemiBold' }}>{isInCart(item.id) ? 'Remove' : 'Add'}</Text></TouchableOpacity>
                </View>
              </View>
            </View>
          ))}
        ...
        {activeSection === 'gc' && <View className="pb-8">
          <Text className="text-xl text-black mb-4" style={{ fontFamily: 'Poppins_700Bold' }}>General Contractors</Text>
          {filteredGCs.map((gc) => (
            <View key={gc.id} className="bg-white rounded-3xl mb-4 overflow-hidden border border-gray-200">
              <View className="flex-row p-4">
                <Image source={{ uri: gc.image }} className="w-20 h-20 rounded-2xl bg-gray-100" resizeMode="cover" />
                <View className="flex-1 ml-4 justify-center">
                  <View className="flex-row items-center mb-1"><Text className="text-lg text-black" style={{ fontFamily: 'Poppins_700Bold' }}>{gc.name}</Text>{gc.verified && <View className="ml-2 bg-black rounded-full px-2 py-0.5"><Text className="text-white text-xs" style={{ fontFamily: 'Poppins_600SemiBold' }}>✓</Text></View>}</View>
        ...
        {activeSection === 'sub' && <View className="pb-8">
          ...
          {filteredSubs.map((sub) => (
            <View key={sub.id} className="bg-white rounded-3xl mb-4 overflow-hidden border border-gray-200">
              <View className="flex-row p-4">
                <Image source={{ uri: sub.image }} className="w-20 h-20 rounded-2xl bg-gray-100" resizeMode="cover" />
                <View className="flex-1 ml-4 justify-center">
                  <View className="flex-row items-center mb-1"><Text className="text-lg text-black" style={{ fontFamily: 'Poppins_700Bold' }}>{sub.name}</Text>{sub.verified && <View className="ml-2 bg-black rounded-full px-2 py-0.5"><Text className="text-white text-xs" style={{ fontFamily: 'Poppins_600SemiBold' }}>✓</Text></View>}</View>
