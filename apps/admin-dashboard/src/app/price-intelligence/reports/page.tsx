'use client';

import { useState } from 'react';
import Link from 'next/link';
import { useSearchParams } from 'next/navigation';
import PiPageHeader from '@/components/price-intelligence/PiPageHeader';
import PiEmptyState from '@/components/price-intelligence/PiEmptyState';
import StatusBadge from '@/components/price-intelligence/StatusBadge';
import { usePiReports } from '@/hooks/usePriceIntelligence';

const PAGE_SIZE = 25;

function formatMoney(value: unknown) {
  if (value == null || value === '') return '—';
  const n = Number(value);
  if (Number.isNaN(n)) return String(value);
  return `₦${n.toLocaleString()}`;
}

export default function PiReportsPage() {
  const searchParams = useSearchParams();
  const [page, setPage] = useState(0);
  const [outcome, setOutcome] = useState(searchParams.get('outcome') ?? '');
  const { data, isLoading, isError, error } = usePiReports({
    take: PAGE_SIZE,
    skip: page * PAGE_SIZE,
    outcome: outcome || undefined,
  });

  const total = data?.total ?? 0;
  const pageCount = Math.max(1, Math.ceil(total / PAGE_SIZE));

  return (
    <div className="space-y-4">
      <PiPageHeader
        title="Reports"
        description="Consumer price reports generated in the last delivery window and beyond."
      />

      <div className="sticky top-0 z-10 flex flex-wrap items-end gap-2 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
        <label className="flex flex-col gap-1 text-xs text-gray-500">
          Outcome
          <select
            value={outcome}
            onChange={(e) => {
              setOutcome(e.target.value);
              setPage(0);
            }}
            className="rounded-lg border px-2 py-2 text-sm"
          >
            <option value="">All</option>
            <option value="complete">complete</option>
            <option value="single_source">single_source</option>
            <option value="insufficient_data">insufficient_data</option>
          </select>
        </label>
      </div>

      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        {isLoading ? (
          <p className="p-8 text-center text-sm text-gray-500">Loading reports…</p>
        ) : isError ? (
          <p className="p-8 text-center text-sm text-red-600">
            {error instanceof Error ? error.message : 'Failed to load'}
          </p>
        ) : !data?.items.length ? (
          <PiEmptyState title="No reports yet" description="Generated reports will list here." />
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                <tr>
                  <th className="px-3 py-2">Product</th>
                  <th className="px-3 py-2">Outcome</th>
                  <th className="px-3 py-2">Typical</th>
                  <th className="px-3 py-2">Range</th>
                  <th className="px-3 py-2">Confidence</th>
                  <th className="px-3 py-2">Version</th>
                  <th className="px-3 py-2">Notice</th>
                  <th className="px-3 py-2">Paid</th>
                  <th className="px-3 py-2">Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {data.items.map((report) => {
                  const item = report.items[0];
                  const product =
                    item?.requestItem?.rawProductName ||
                    item?.requestItem?.family?.key ||
                    '—';
                  return (
                    <tr key={report.id} className="hover:bg-gray-50">
                      <td className="px-3 py-2">
                        <p className="font-medium text-gray-900">{product}</p>
                        <Link
                          href={`/price-intelligence/review-queue?q=${report.id}`}
                          className="text-xs text-blue-600 hover:underline"
                        >
                          Find related cases
                        </Link>
                      </td>
                      <td className="px-3 py-2">
                        <StatusBadge status={item?.outcome || report.status} />
                      </td>
                      <td className="px-3 py-2">{formatMoney(item?.typicalPrice)}</td>
                      <td className="px-3 py-2 text-xs">
                        {formatMoney(item?.rangeLow)} – {formatMoney(item?.rangeHigh)}
                      </td>
                      <td className="px-3 py-2 text-xs">{item?.confidence || '—'}</td>
                      <td className="px-3 py-2">v{report.currentVersion}</td>
                      <td className="max-w-[160px] truncate px-3 py-2 text-xs text-gray-500">
                        {report.customerUpdateNotice || '—'}
                      </td>
                      <td className="px-3 py-2 text-xs">
                        {report.request?.paymentOrderId ? (
                          <span className="font-medium text-green-700">Paid</span>
                        ) : (
                          <span className="text-gray-500">Free / unpaid</span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-xs text-gray-500">
                        {new Date(report.generatedAt || report.createdAt).toLocaleString()}
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
          </div>
        )}
        <div className="flex items-center justify-between border-t px-4 py-3 text-sm">
          <span className="text-gray-500">{total} reports</span>
          <div className="flex gap-2">
            <button
              type="button"
              disabled={page <= 0}
              onClick={() => setPage((p) => p - 1)}
              className="rounded-lg border px-3 py-1.5 disabled:opacity-40"
            >
              Previous
            </button>
            <span className="text-xs text-gray-500 self-center">
              Page {page + 1} / {pageCount}
            </span>
            <button
              type="button"
              disabled={page + 1 >= pageCount}
              onClick={() => setPage((p) => p + 1)}
              className="rounded-lg border px-3 py-1.5 disabled:opacity-40"
            >
              Next
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
