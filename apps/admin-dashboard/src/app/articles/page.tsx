...
import ServicePagesPanel from '@/components/content/ServicePagesPanel';
import {
  useCmsArticles,
  type CmsArticle,
...
type ContentTab = 'articles' | 'service-pages';
...
export default function ArticlesAdminPage() {
  ...
  const activeTab: ContentTab =
    searchParams.get('tab') === 'service-pages' ? 'service-pages' : 'articles';
  ...
  const setTab = (tab: ContentTab) => {
    router.replace(tab === 'service-pages' ? '/articles?tab=service-pages' : '/articles');
  };
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold font-poppins">Content</h1>
          <p className="text-gray-500 mt-1">
            {activeTab === 'articles'
              ? 'Medium-style publishing for homeowner and GC domains.'
              : 'Manage Lagos and Nigeria service landing pages — copy, images, CTAs, and SEO.'}
          </p>
        </div>
        {activeTab === 'articles' ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          ...
          <div className={`text-xs mt-1 ${activeTab === 'articles' ? 'text-white/80' : 'text-gray-500'}`}>
            Homeowner and GC blog posts
          </div>
        </button>
        <button
          type="button"
          onClick={() => setTab('service-pages')}
          className={`rounded-xl border p-4 text-left transition ${
        ...
        >
          <div className="text-sm font-semibold">Service pages</div>
          <div
            className={`text-xs mt-1 ${activeTab === 'service-pages' ? 'text-white/80' : 'text-gray-500'}`}
          >
            Lagos and Nigeria repair landing pages
          </div>
        </button>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {AUDIENCE_OPTIONS.map((opt) => (
              ...
              >
                ...
                <div
                  className={`text-xs mt-1 ${audience === opt.value ? 'text-white/80' : 'text-gray-500'}`}
                >
                  {opt.hint}
                </div>
              </button>
          ...
          <div className="bg-white rounded-xl shadow">
            ...
            {isLoading ? (
              ...
              <div className="p-8 text-center text-gray-500">No articles yet for this audience.</div>
            ) : (
