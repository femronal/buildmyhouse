// scripts/price-catalogue-roundtrip-check.ts
...
async function main() {
  ...
  for (const loc of LOCATIONS) {
    ...
    if (row) {
      ...
      check(
        (row.parent?.code ?? void 0) === loc.parentKey,
  ...
  console.log(
    `\u2705 Round-trip check passed: ${LEVEL1_FAMILIES.length} families, ${UNITS.length} units, ${CONVERSION_RULES.length} conversion rules, ${LOCATIONS.length} locations, ${SERVICE_FAMILIES.length} service families verified against the database.`
  );
}
main().catch((err) => {
